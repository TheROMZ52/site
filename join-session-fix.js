...
(function(){
  ...
  async function loadAllRequests(){
    ...
    const accounts={};
    if(ids.length){
      const {data:rows,error:e2}=await sb.from('accounts').select('id,username,rank,team_status').in('id',ids);
      if(e2) console.error(e2); else (rows||[]).forEach(a=>accounts[a.id]=a);
    }
  ...
  async function renderAdminPanel(){
    ...
    if(!root||!isReviewer()) return false;
    root.innerHTML=`<div class="kz-join-shell" id="requests">
      <div class="section-title top"><h2>درخواست‌های عضویت</h2><p>مرکز بررسی درخواست‌ها و گفت‌وگوی تیم با متقاضی‌ها.</p></div>
      <div id="kzAdminRequests"><div class="loading-note">در حال بارگذاری درخواست‌ها...</div></div>
    </div>`;
    const data=await loadAllRequests();
    const host=document.getElementById('kzAdminRequests');
    if(!host) return true;
    ...
    const render=async()=>{
      const selected=data.requests.find(r=>r.id===selectedId)||data.requests[0];
      selectedId=selected.id;
      const acc=data.accounts[selected.account_id]||{};
      const {data:messages,error:me}=await sb.from('team_join_messages').select('*').eq('request_id',selected.id).order('created_at',{ascending:true});
      if(me) console.error(me);
      host.innerHTML=`<div class="kz-admin-layout">
        <aside class="kz-ticket-list">
          <div class="kz-ticket-list-head"><div><strong>درخواست‌ها</strong><p>${data.requests.length} تیکت</p></div><button class="link-btn" id="kzReqRefresh">↻ بروزرسانی</button></div>
          <div class="kz-list-items">${data.requests.map(r=>{const a=data.accounts[r.account_id]||{};return `<button class="kz-ticket-item ${r.id===selected.id?'active':''}" data-request-id="${esc(r.id)}"><span><strong>${esc(applicantName(r,a))}</strong><small>${esc(a.username||r.rubika_id||'')}</small></span><span class="kz-status ${statusClass(r.status)}">${esc(statusText(r.status))}</span></button>`;}).join('')}</div>
        </aside>
        <section class="kz-ticket-detail">
          ...
          <div class="kz-admin-actions"><button class="btn primary" data-action="approve">✓ پذیرش</button><button class="btn" data-action="reviewing">در حال بررسی</button><button class="btn" data-action="waiting_applicant">منتظر پاسخ</button><button class="btn kz-danger" data-action="reject">✕ رد درخواست</button><button class="btn" data-action="closed">بستن تیکت</button></div>
          ${requestFields(selected,acc)}
          ...
          ${ACTIVE.includes(selected.status)?`<form id="kzStaffReply" class="kz-reply"><label for="kzStaffReplyInput">پیام برای متقاضی</label><textarea id="kzStaffReplyInput" rows="4" required placeholder="پیامت رو برای متقاضی بنویس..."></textarea><button class="btn primary" type="submit">ارسال پیام</button><div id="kzStaffReplyMsg"></div></form>`:''}
          <div id="kzAdminMsg"></div>
        </section>
      </div>`;

      host.querySelectorAll('.kz-ticket-item').forEach(b=>b.addEventListener('click',()=>{selectedId=b.dataset.requestId;render();}));
      const refresh=document.getElementById('kzReqRefresh');
      if(refresh) refresh.addEventListener('click',async()=>{const fresh=await loadAllRequests();data.requests=fresh.requests;data.accounts=fresh.accounts;await render();});
      host.querySelectorAll('[data-action]').forEach(b=>b.addEventListener('click',()=>changeStatus(selected,b.dataset.action)));
      const reply=document.getElementById('kzStaffReply');
      if(reply) reply.addEventListener('submit',async e=>{
        e.preventDefault();
        const input=document.getElementById('kzStaffReplyInput');
        const msg=document.getElementById('kzStaffReplyMsg');
        const text=input.value.trim(); if(!text) return;
        const {error}=await sb.from('team_join_messages').insert([{request_id:selected.id,account_id:selected.account_id,sender_role:'staff',message:text}]);
    ...
    async function changeStatus(req,action){
      ...
      const msg=document.getElementById('kzAdminMsg');
      ...
      const accountUpdate=newStatus==='approved'?{rank:'member',team_status:'approved'}:{team_status:newStatus==='rejected'?'rejected':'reviewing'};
  ...
  async function renderApplicant(){
    const root=document.getElementById('kzJoinApp');
    if(!root || !window.currentUser || isReviewer()) return;
  ...
  function bindForm(root){
    ...
    form.addEventListener('submit',async e=>{
      ...
      const payload={id:typeof kzNewId==='function'?kzNewId('req'):'req-'+Date.now(),account_id:currentUser.id,status:'reviewing',name:values.first_name,last_name:values.last_name,rubika_id:values.rubika_id,age:values.age,city:values.city,other_games:values.other_games,skill_level:values.skill_level,gaming_years:values.gaming_years,weekly_activity:values.weekly_activity,voice_chat:values.voice_chat,why_join:values.why_join,contribution:values.contribution,conflict_response:values.conflict_response,how_found_us:values.how_found_us,info_confirmed:values.info_confirmed};
      ...
      const {data:updated}=await sb.from('accounts').update({team_status:'reviewing',rank:'guest'}).eq('id',currentUser.id).select('*').maybeSingle();
  ...
  async function boot(){
    ...
    if(window.currentUser && isReviewer()) await renderAdminPanel();
    else if(window.currentUser) await renderApplicant();
  }

  window.kzRenderJoinPage=async()=>{
    if(isReviewer()) return renderAdminPanel();
