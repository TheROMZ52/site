// KillZone UI domain — merged from mobile-nav.js, hero-console.js, ui-rehydrate.js,
// member-visibility-fix.js and guest-accounts.js.
(function () {
  "use strict";

  // ===== Former mobile-nav.js =====
  ...
  async function loadConsole() {
    ...
    try {
      ...
      console.warn("KillZone field console load failed", e);
    }
  }
  ...
  async function refreshUI() {
    ...
    try {
      ...
      if (
        ...
        typeof window.renderMembersPage === "function"
      )
        await window.renderMembersPage();
      if (
        document.getElementById("gamesContainer") &&
        typeof window.renderGamesPage === "function"
      )
        await window.renderGamesPage();
      if (
        document.getElementById("regForm") &&
        typeof window.renderRegisterPage === "function"
      )
        await window.renderRegisterPage();
      if (
  ...
  function scheduleRehydrate() {
    ...
    rehydrateTimer = setTimeout(refreshUI, 60);
  }
  function bootRehydrate() {
    [0, 250, 800, 1600, 3000].forEach((ms) =>
      setTimeout(scheduleRehydrate, ms),
    );
    ...
    window.addEventListener("kz:session-ready", scheduleRehydrate);
    window.addEventListener("kz:rehydrate", scheduleRehydrate);
    window.kzRehydrateUI = scheduleRehydrate;
  }

  // ===== Former member-visibility-fix.js =====
  function isStaff() {
    const u = window.currentUser;
    return !!(
      u && ["admin", "developer", "co_owner", "owner"].includes(u.rank)
    );
  }
  function cleanMembersPage() {
    ...
    container.querySelectorAll(".rank-section").forEach((section) => {
      ...
      if (!heading) return;
      if (heading.textContent.trim() === "مهمان" && !allowGuests)
        section.remove();
    });
  }
  function bootMemberVisibility() {
    cleanMembersPage();
    const container = document.getElementById("membersContainer");
    if (!container) return;
    new MutationObserver(cleanMembersPage).observe(container, {
      childList: true,
      subtree: true,
    });
  }

  // ===== Former guest-accounts.js =====
  ...
  const guestStatus = (status) =>
    ({
      ...
      waiting_applicant: "منتظر پاسخ متقاضی",
      approved: "تأیید شده",
      rejected: "رد شده",
      closed: "بسته شده",
    })[status] ||
  ...
  async function approveGuest(accountId) {
    if (!guestStaff() || typeof sb === "undefined") return;
    ...
    if (!account.data)
      throw new Error("اکانت مهمان پیدا نشد یا قبلاً تغییر کرده.");
    if (!confirm(`اکانت «${account.data.username}» به عضو تأییدشده تبدیل شود؟`))
      return;
    const req = await sb
    ...
    if (req.data?.[0]) {
      const updateRequest = await sb
        .from("team_join_requests")
        .update({ status: "approved" })
        .eq("id", req.data[0].id);
      if (updateRequest.error)
        console.warn("Guest request status sync failed:", updateRequest.error);
    }
    await showGuests();
  }
  async function deleteGuest(accountId) {
    ...
    if (account.error) throw account.error;
    if (!account.data)
      throw new Error("اکانت مهمان پیدا نشد یا قبلاً حذف/تأیید شده.");
    if (
      !confirm(
        `اکانت مهمان «${account.data.username}» و درخواست‌های مرتبط حذف شوند؟\n\nاین کار قابل برگشت نیست.`,
      )
    )
      return;
    const req = await sb
  ...
  async function guestAction(accountId, kind, button) {
  ...
  async function showGuests() {
    const container = document.getElementById("membersContainer");
    if (!container || !guestStaff() || typeof sb === "undefined") return;
    container.querySelector(".kz-guest-section")?.remove();
    const section = document.createElement("div");
    section.className = "rank-section kz-guest-section";
    section.innerHTML =
      '<div class="rank-heading"><span class="tier">▲</span><h3>مهمان‌ها</h3><div class="rule"></div></div><div class="member-grid kz-guest-grid"><div class="loading-note">در حال بارگذاری اکانت‌های مهمان...</div></div>';
    container.prepend(section);
    const grid = section.querySelector(".kz-guest-grid");
    const { data, error } = await sb
      .from("accounts")
      .select("id,username,rank,game,photo,team_status")
      .eq("rank", "guest")
      .order("username");
    if (error) {
      grid.innerHTML =
        '<div class="empty-note">بارگذاری مهمان‌ها ناموفق بود.</div>';
      console.error(error);
      return;
    }
    if (!data?.length) {
      grid.innerHTML = '<div class="empty-note">اکانت مهمانی وجود نداره.</div>';
      return;
    }
    grid.innerHTML = "";
    data.forEach((m) => {
      ...
      const actions = `<div class="guest-actions"><button class="btn primary small kz-guest-approve" type="button" data-account-id="${guestEsc(m.id)}">✓ تأیید اکانت</button><button class="btn danger small kz-guest-delete" type="button" data-account-id="${guestEsc(m.id)}">× حذف اکانت</button></div>`;
      card.innerHTML = `${m.photo ? `<img class="avatar" src="${guestEsc(m.photo)}" alt="${guestEsc(m.username)}" loading="lazy" decoding="async">` : `<div class="avatar">${guestInitials(m.username)}</div>`}<h4>${guestEsc(m.username)}</h4><div class="rank-badge"><span class="tier">▲</span> مهمان</div><div class="game-tag">${guestEsc(m.game || "—")}</div><div class="guest-status">وضعیت عضویت: ${guestEsc(guestStatus(m.team_status))}</div>${actions}`;
      grid.appendChild(card);
    });
    grid
      .querySelectorAll(".kz-guest-approve")
      .forEach((button) =>
        button.addEventListener("click", () =>
          guestAction(button.dataset.accountId, "approve", button),
        ),
      );
    grid
      .querySelectorAll(".kz-guest-delete")
      .forEach((button) =>
        button.addEventListener("click", () =>
          guestAction(button.dataset.accountId, "delete", button),
        ),
      );
  }
  function hideGuests() {
  ...
  function renderGuestButton() {
    ...
    button.textContent = shown
      ? "👁 مخفی‌کردن مهمان‌ها"
      : "👥 مشاهده اکانت‌های مهمان";
    button.setAttribute("aria-pressed", String(shown));
    if (!guestWired) {
      guestWired = true;
      button.addEventListener("click", async () => {
        if (!guestStaff()) return;
        const next = document.body.dataset.kzShowGuests !== "1";
        button.disabled = true;
        try {
          if (next) {
            document.body.dataset.kzShowGuests = "1";
            await showGuests();
          } else hideGuests();
        } finally {
          button.disabled = false;
  ...
  function boot() {
    bootMobileNav();
    if (document.querySelector(".field-console")) bootHero();
    if (document.getElementById("regForm")) bootRehydrate();
    if (document.getElementById("membersContainer")) {
      bootMemberVisibility();
      bootGuestAccounts();
    }
  }
