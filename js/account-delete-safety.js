    ...
    document.addEventListener(
      ...
      async function (event) {
        ...
        const confirmation = prompt(
          `برای حذف دائمی اکانت «${user.username}»، نام‌کاربری را دقیقاً وارد کن:`,
        );
        if (confirmation !== user.username) return;
        if (
          !confirm(
            "مطمئنی؟ اکانت، درخواست‌های عضویت، پیام‌ها، حضور و دستاوردها حذف می‌شن و این کار قابل برگشت نیست.",
          )
        )
          return;
