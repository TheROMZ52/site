...
(function () {
  ...
  const norm = (v) =>
    String(v ?? "")
      ...
      .toLowerCase();
  const staff = () =>
    !!(window.currentUser && STAFF.has(window.currentUser.rank));
  const esc = (v) =>
  ...
  function isOnline(id) {
    ...
    return !!(
      p?.last_seen && Date.now() - new Date(p.last_seen).getTime() <= ONLINE_MS
    );
  }
  function gamesOf(a) {
  ...
  function updateStats() {
    const total = document.getElementById("membersTotal"),
      online = document.getElementById("membersOnline"),
      games = document.getElementById("membersGames");
    if (total) total.textContent = accounts.length;
    if (online)
      online.textContent = accounts.filter((a) => isOnline(a.id)).length;
    if (games)
      games.textContent = new Set(accounts.flatMap(gamesOf).map(norm)).size;
  }
  function populateGames() {
    ...
    const values = [
      ...new Map(accounts.flatMap(gamesOf).map((g) => [norm(g), g])).entries(),
  ...
  function addPresence(card, a) {
    ...
    if (!isOnline(a.id))
      ...
      const labels = {
        ...
        info = labels[p?.status] || labels.ready;
  ...
  function sortCards(cards) {
    ...
    return [...cards].sort((x, y) => {
      const a = getCardAccount(x),
        b = getCardAccount(y);
      if (mode === "online")
        return (
          Number(isOnline(b?.id)) - Number(isOnline(a?.id)) ||
  ...
  function applyFilters() {
    const r = root();
    if (!r) return;
    const grid = r.querySelector(":scope > .kz-profile-grid");
    if (!grid) return;
    const q = norm(document.getElementById("memberSearch")?.value),
      game = norm(document.getElementById("memberGameFilter")?.value || "all"),
      status = document.getElementById("memberStatusFilter")?.value || "all",
      cards = [...grid.querySelectorAll(":scope > .kz-profile-card")];
    cards.forEach((card) => {
      const a = getCardAccount(card),
        text = norm([a?.username, a?.bio, a?.game].join(" ")),
        gameOk = game === "all" || gamesOf(a).some((g) => norm(g) === game),
        statusOk =
          status === "all" ||
          (status === "online" ? isOnline(a?.id) : !isOnline(a?.id));
      card.hidden = !(text.includes(q) && gameOk && statusOk);
      addPresence(card, a);
    });
    sortCards(cards).forEach((c) => grid.appendChild(c));
    const visible = cards.filter((c) => !c.hidden),
      count = document.getElementById("membersResultCount");
    if (count)
      count.textContent = `نمایش ${visible.length} از ${cards.length} عضو`;
  ...
  function wireControls() {
    const search = document.getElementById("memberSearch"),
      clear = document.getElementById("memberSearchClear"),
      game = document.getElementById("memberGameFilter"),
      status = document.getElementById("memberStatusFilter"),
      sort = document.getElementById("memberSort"),
      reset = document.getElementById("memberResetFilters");
    if (search && !search.dataset.kzBound) {
      search.dataset.kzBound = "1";
      search.addEventListener("input", applyFilters);
      search.addEventListener("compositionend", applyFilters);
    }
    if (clear && !clear.dataset.kzBound) {
      clear.dataset.kzBound = "1";
    ...
    [game, status, sort].forEach((el) => {
      if (el && !el.dataset.kzBound) {
        el.dataset.kzBound = "1";
        el.addEventListener("change", applyFilters);
      }
    });
    if (reset && !reset.dataset.kzBound) {
      reset.dataset.kzBound = "1";
  ...
  async function guestManager() {
    ...
    let o = document.getElementById("kzGuestManager");
    if (!o) {
      o = document.createElement("div");
      o.className = "overlay kz-community-overlay";
      o.id = "kzGuestManager";
      o.innerHTML =
        '<div class="modal kz-modal-panel" role="dialog" aria-modal="true" aria-labelledby="kzGuestTitle"><button class="close" id="kzGuestClose" aria-label="بستن">✕</button><div class="kz-profile-section-title"><h4 id="kzGuestTitle">اکانت‌های مهمان</h4><span>GUEST ACCOUNTS</span></div><div id="kzGuestList"></div></div>';
      document.body.appendChild(o);
      o.addEventListener("click", (e) => {
        if (e.target === o) o.classList.remove("show");
      });
      o.querySelector("#kzGuestClose").addEventListener("click", () =>
        o.classList.remove("show"),
      );
    }
    const box = o.querySelector("#kzGuestList");
    box.innerHTML =
    ...
    if (error) {
      box.innerHTML =
        '<div class="kz-community-empty">دریافت اکانت‌های مهمان ناموفق بود.</div>';
      return;
    }
    if (!data?.length) {
      box.innerHTML =
    ...
    box.querySelectorAll(".kz-runtime-guest-card").forEach((card) => {
      ...
      card.querySelector('[data-action="delete"]').onclick = () => {
        const b = card.querySelector('[data-action="delete"]');
  ...
  function wireStaff() {
    const add = document.getElementById("addMemberBtn"),
      guests = document.getElementById("guestAccountsBtn");
  ...
  function boot() {
    ...
    observer = new MutationObserver(() => {
      if (r.querySelector(":scope > .kz-profile-grid")) applyFilters();
    });
