...
(function(){
  ...
  async function loadAccounts(){
    ...
    if(error){console.warn('KZ member runtime accounts:',error);return []}
    return data||[];
  }
  async function loadPresence(){
    if(!window.sb)return [];
    const {data,error}=await sb.from('member_presence').select('account_id,status,game,status_text,last_seen');
    if(error){console.warn('KZ member runtime presence:',error);return []}
    return data||[];
  }
  ...
  function paintPresence(){
    ...
    const byId=new Map(cachedPresence.map(x=>[x.account_id,x]));
    const byName=new Map(cachedAccounts.map(x=>[norm(x.username),x]));
    r.querySelectorAll('.kz-profile-card').forEach(card=>{
      card.querySelector('.kz-runtime-presence')?.remove();
      const account=byName.get(norm(card.querySelector('.kz-profile-name')?.textContent));
      if(!account)return;
      const p=byId.get(account.id);const age=p?.last_seen?Date.now()-new Date(p.last_seen).getTime():Infinity;const online=Number.isFinite(age)&&age<=ONLINE_MS;
      const box=document.createElement('div');box.className='kz-runtime-presence';
      if(!online){box.innerHTML='<span class="kz-presence-dot offline"></span><span>آفلاین</span>'}
      else{const labels={playing:['در حال بازی','🎮'],competitive:['در حال رقابت','🏆'],ready:['آماده','🟢'],busy:['مشغول','🔴'],away:['AFK','💤']};const info=labels[p.status]||labels.ready;box.innerHTML='<span class="kz-presence-dot"></span><span>آنلاین · '+info[1]+' '+info[0]+'</span>'+(p.game?'<small>'+esc(p.game)+'</small>':'')+(p.status_text?'<small>'+esc(p.status_text)+'</small>':'')}
      card.querySelector('.kz-profile-main')?.appendChild(box);
    });
  }

  function wireGuestButton(){
    const b=document.getElementById('guestAccountsBtn');if(!b)return;
    b.style.display=staff()?'inline-flex':'none';
    if(b.dataset.kzRuntimeBound)return;
    b.dataset.kzRuntimeBound='1';
    b.addEventListener('click',async()=>{
      const show=b.dataset.shown==='1';
      if(show){b.dataset.shown='0';delete document.body.dataset.kzShowGuests;root()?.querySelector('.kz-runtime-guests')?.remove();b.textContent='👥 مشاهده اکانت‌های مهمان';return}
      b.dataset.shown='1';document.body.dataset.kzShowGuests='1';b.textContent='👁 مخفی‌کردن مهمان‌ها';await renderGuests();
    });
  }

  async function renderGuests(){
    const r=root();if(!r||!staff()||!window.sb)return;
    r.querySelector('.kz-runtime-guests')?.remove();
    const {data,error}=await sb.from('accounts').select('id,username,game').eq('rank','guest').order('username');
    if(error){console.warn('KZ guest runtime:',error);return}
    const sec=document.createElement('section');sec.className='kz-runtime-guests';sec.innerHTML='<div class="kz-profile-section-title"><h3>اکانت‌های مهمان</h3><span>GUEST ACCOUNTS</span></div><div class="kz-runtime-guest-grid"></div>';
    const grid=sec.querySelector('.kz-runtime-guest-grid');
    if(!data?.length){grid.innerHTML='<div class="kz-community-empty">اکانت مهمانی وجود نداره.</div>'}
    else data.forEach(m=>{
      const card=document.createElement('article');card.className='kz-runtime-guest-card';card.innerHTML='<div class="avatar">'+esc((m.username||'?').slice(0,2).toUpperCase())+'</div><div class="body"><div class="name">'+esc(m.username)+'</div><div class="meta">'+esc(m.game||'بازی ثبت نشده')+' · مهمان</div></div><div class="actions"><button class="btn primary small" data-action="approve" type="button">✓ تأیید</button><button class="btn ghost small" data-action="delete" type="button">حذف</button></div>';
      card.querySelector('[data-action="approve"]').onclick=async()=>{if(!confirm('اکانت «'+m.username+'» تأیید و به عضو تبدیل شود؟'))return;const {error:e}=await sb.from('accounts').update({rank:'member',team_status:'approved'}).eq('id',m.id).eq('rank','guest');if(e){alert('تأیید اکانت انجام نشد.');return}await refresh(true)};
      card.querySelector('[data-action="delete"]').onclick=async()=>{if(!confirm('اکانت «'+m.username+'» حذف شود؟'))return;const {error:e}=await sb.from('accounts').delete().eq('id',m.id).eq('rank','guest');if(e){alert('حذف اکانت انجام نشد.');return}await refresh(true)};
  ...
  async function refresh(force){
    const r=root();if(!r||!window.sb)return;
    cachedAccounts=await loadAccounts();cachedPresence=await loadPresence();
    const sig=signature(cachedAccounts);
    if(force||sig!==lastSignature){if(r.querySelector(':scope > .kz-profile-grid'))await groupCards();lastSignature=sig}
    paintPresence();wireGuestButton();
    if(staff()&&document.body.dataset.kzShowGuests==='1')await renderGuests();
  }
  function queue(force){if(queued&&!force)return;queued=true;requestAnimationFrame(async()=>{queued=false;await refresh(!!force)})}
  function boot(){
    styles();const r=root();if(!r){setTimeout(boot,250);return}
    observer=new MutationObserver(()=>{if(busy)return;if(r.querySelector(':scope > .kz-profile-grid')&&!r.querySelector(':scope > .kz-community-rank-section'))queue(true)});
    observer.observe(r,{childList:true,subtree:true});
    wireGuestButton();queue(true);
    window.addEventListener('kz:session-changed',()=>queue(true));
    window.addEventListener('kz:community-refresh',()=>queue(true));
    setInterval(()=>queue(false),5000);
  }
