...
function rankTier(key) {
  const i = RANKS.findIndex((x) => x.key === key);
  return i < 0 ? 1 : i + 1;
}
function rankChevrons(key) {
  return "▲".repeat(rankTier(key));
}
...
function isStaff(u) {
  return !!(u && ADMIN_RANKS.includes(u.rank));
}
...
function saveSession(acc) {
  localStorage.setItem(SESSION_KEY, JSON.stringify({ id: acc.id }));
}
...
async function registerUser(username, password, game, photo) {
  ...
  const acc = {
    ...
    username,
    pass_hash: await hashPass(password),
    rank: "new_member",
    game: game || "",
    photo: photo || "",
    is_admin: false,
  ...
  if (error) {
    console.error(error);
    return { ok: false, msg: "خطا در ثبت‌نام. دوباره تلاش کن." };
  }
    ...
    await sb.from("game_modes").insert([
      {
        ...
        title: "اسکای‌بلاک",
        description:
          "بازسازی از صفر روی یه جزیره کوچیک؛ منابع، اقتصاد و پیشرفت تیمی.",
        maps: "Island Reset, Economy",
        sort_order: 1,
      },
      {
        id: "mode-2",
        block_id: "blk-mc",
        title: "اسکای‌وارز",
        description: "نبرد سریع روی جزیره‌های معلق؛ لوت کن، آماده شو، حمله کن.",
        maps: "Solo, Teams",
        sort_order: 2,
      },
      {
        ...
        block_id: "blk-mc",
        title: "بدوارز",
        description: "دفاع از تخت، خرید آپگرید، و حذف تیم‌های رقیب یکی‌یکی.",
        maps: "4-Team, 8-Team",
        sort_order: 3,
      },
      {
        ...
        title: "سروایول",
        description:
          "سرور اصلی تیم برای ساخت‌وساز بلندمدت روی مپ‌های محبوب جامعه.",
        maps: "محبوب #1, محبوب #2, محبوب #3",
        sort_order: 4,
      },
      {
        id: "mode-5",
        block_id: "blk-cs",
        title: "Respawn Deathmatch",
        description:
          "مچ سریع؛ بعد از مرگ دوباره Spawn می‌شوی و بازی تا پایان تایمر ادامه دارد.",
        maps: "Respawn, Kill Feed",
        sort_order: 1,
      },
      {
        id: "mode-6",
        block_id: "blk-cs",
        title: "تمرینی",
        description: "تمرین آیم و شلیک در میدان سه‌بعدی.",
        maps: "Dust II 3D, Practice",
        sort_order: 2,
      },
    ]);
  }
...
async function fetchGameData() {
  const { data: blocks, error: e1 } = await sb
    .from("game_blocks")
    .select("*")
    .order("sort_order");
  const { data: modes, error: e2 } = await sb
    .from("game_modes")
    .select("*")
    .order("sort_order");
...
function wireLoginModal() {
  ...
  document.getElementById("loginSubmit").addEventListener("click", async () => {
    ...
    if (!username || !pass) {
      msg.innerHTML =
        '<div class="form-msg err">یوزرنیم و رمز رو وارد کن.</div>';
      return;
    }
    const res = await loginUser(username, pass);
    if (!res.ok) {
      msg.innerHTML = `<div class="form-msg err">${escapeHtml(res.msg)}</div>`;
      return;
    }
...
function buildMemberCard(m, staff, accountsCache) {
  ...
  if (staff) {
    card
      .querySelector(".edit")
      .addEventListener("click", () => openMemberModal(m.id, accountsCache));
    card
      .querySelector(".del")
...
async function renderMembersPage() {
  ...
  if (accounts.length === 0) {
    container.innerHTML =
      '<div class="empty-note">هنوز عضوی ثبت‌نام نکرده.</div>';
  } else {
    ...
    orderHighToLow.forEach((rankKey) => {
      ...
      group.forEach((m) =>
        grid.appendChild(buildMemberCard(m, staff, accounts)),
      );
...
function openMemberModal(id, accounts) {
  ...
  if (!overlay) return;
  document.getElementById("memberModalTitle").textContent = id
    ? "ویرایش عضو"
    : "افزودن عضو دستی";
...
async function loadMemberGameChecks(selected) {
  const box = document.getElementById("mGamesBox");
  if (!box) return;
  ...
  box.innerHTML =
    names
      .map(
        (name) =>
          `<label class="kz-game-check"><input type="checkbox" value="${escapeHtml(name)}" ${set.has(name) ? "checked" : ""}> ${escapeHtml(name)}</label>`,
      )
      .join("") || '<div class="hint">هنوز بازی‌ای تعریف نشده.</div>';
}
function wireMemberModal() {
  const overlay = document.getElementById("memberOverlay");
  if (!overlay || overlay.dataset.kzCoreReady === "1") return;
  ...
  document
    .getElementById("mPhotoFile")
    ?.addEventListener("change", async (e) => {
      const file = e.target.files?.[0];
      if (!file) return;
      const url = await uploadPhoto(file, (type, text) => {
        const msg = document.getElementById("memberMsg");
        if (msg)
          msg.innerHTML = `<div class="form-msg ${type}">${escapeHtml(text)}</div>`;
      });
...
async function saveMember() {
  ...
  if (!name) {
    if (msg)
      msg.innerHTML = '<div class="form-msg err">نام‌کاربری رو وارد کن.</div>';
    return;
  }
  const games = [...document.querySelectorAll("#mGamesBox input:checked")]
    .map((x) => x.value)
    .join("، ");
  const data = {
    username: name,
  ...
  try {
    ...
    if (editingMemberId) {
      ...
      data.pass_hash =
        data.pass_hash ||
        (await hashPass(Math.random().toString(36).slice(2, 10)));
      data.is_admin = ADMIN_RANKS.includes(data.rank);
      data.team_status = data.rank === "guest" ? "none" : "approved";
      const { error } = await sb.from("accounts").insert([data]);
      if (error) throw error;
    }
    ...
    console.error(e);
    if (msg)
      msg.innerHTML = '<div class="form-msg err">ذخیره عضو ناموفق بود.</div>';
  }
}
...
function gameCard(block, modes, staff) {
  ...
  blockModes.forEach((mode) => {
    ...
    const chips = String(mode.maps || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
      .map((s) => `<span>${escapeHtml(s)}</span>`)
      .join("");
    card.innerHTML = `<h4>${escapeHtml(mode.title)}</h4><p>${escapeHtml(mode.description || "")}</p><div class="maps">${chips}</div>${staff ? `<div class="member-actions"><button class="icon-btn edit-mode" data-id="${escapeHtml(mode.id)}">ویرایش</button><button class="icon-btn del del-mode" data-id="${escapeHtml(mode.id)}">حذف</button></div>` : ""}`;
    grid.appendChild(card);
  });
  wrap
    .querySelectorAll(".edit-block")
    .forEach((b) =>
      b.addEventListener("click", () =>
        openBlockModal(b.dataset.id, window.__kzGameCache?.blocks || []),
      ),
    );
  wrap
    .querySelectorAll(".del-block")
    .forEach((b) =>
      b.addEventListener("click", () => deleteBlock(b.dataset.id)),
    );
  wrap
    .querySelectorAll(".add-mode")
    .forEach((b) =>
      b.addEventListener("click", () =>
        openModeModal(null, b.dataset.block, window.__kzGameCache?.modes || []),
      ),
    );
  wrap
    ...
    .forEach((b) =>
      b.addEventListener("click", () =>
        openModeModal(b.dataset.id, null, window.__kzGameCache?.modes || []),
      ),
    );
  wrap
    .querySelectorAll(".del-mode")
    .forEach((b) =>
      b.addEventListener("click", () => deleteMode(b.dataset.id)),
    );
  return wrap;
}
async function renderGamesPage() {
  ...
  if (!container) return;
  container.innerHTML =
    '<div class="loading-note">در حال بارگذاری بازی‌ها...</div>';
  const data = await fetchGameData();
  window.__kzGameCache = data;
  container.innerHTML = "";
  const staff = isStaff(currentUser);
  if (!staff) {
    container.innerHTML =
      '<div class="empty-note">مدیریت مودها فقط برای اعضای تیم مدیریت قابل مشاهده است.</div>';
    document.getElementById("addGameBtn")?.style.setProperty("display", "none");
    return;
  }
  if (!data.blocks.length) {
    container.innerHTML =
      '<div class="empty-note">هنوز بازی‌ای اضافه نشده.</div>';
    return;
  }
  data.blocks.forEach((block) =>
    container.appendChild(gameCard(block, data.modes, staff)),
  );
  document
...
function openBlockModal(id, blocks) {
  ...
  document.getElementById("blockModalTitle").textContent = id
    ? "ویرایش بازی"
    : "افزودن بازی";
  document.getElementById("bName").value = b?.name || "";
  document.getElementById("bTag").value = b?.tag || "";
  document.getElementById("bTheme").value = b?.theme || "neutral";
...
function openModeModal(id, blockId, modes) {
  if (!isStaff(currentUser)) return;
  editingModeId = id || null;
  editingModeBlockId = blockId || null;
  const m = (modes || []).find((x) => x.id === id);
  if (id) editingModeBlockId = m?.block_id || null;
  document.getElementById("modeModalTitle").textContent = id
    ? "ویرایش مود"
    : "افزودن مود جدید";
  ...
  document.getElementById("modeMsg").innerHTML = "";
...
function wireGameModals() {
  ...
  if (bo && !bo.dataset.kzCoreReady) {
    bo.dataset.kzCoreReady = "1";
    document
      .getElementById("blockClose")
      ?.addEventListener("click", () => bo.classList.remove("show"));
    bo.addEventListener("click", (e) => {
      if (e.target === bo) bo.classList.remove("show");
    });
    document
      .getElementById("addGameBtn")
      ?.addEventListener("click", () => openBlockModal(null, []));
    document.getElementById("blockSave")?.addEventListener("click", saveBlock);
  }
  const mo = document.getElementById("gameModeOverlay");
  if (mo && !mo.dataset.kzCoreReady) {
    mo.dataset.kzCoreReady = "1";
    document
      .getElementById("modeClose")
      ?.addEventListener("click", () => mo.classList.remove("show"));
    mo.addEventListener("click", (e) => {
      if (e.target === mo) mo.classList.remove("show");
    });
    document.getElementById("modeSave")?.addEventListener("click", saveMode);
  }
}
async function saveBlock() {
  ...
  if (!name) {
    msg.innerHTML = '<div class="form-msg err">اسم بازی رو وارد کن.</div>';
    return;
  }
  const data = {
    name,
  ...
  try {
    const q = editingBlockId
      ? sb.from("game_blocks").update(data).eq("id", editingBlockId)
      : sb
          .from("game_blocks")
          .insert([
            {
    ...
    const { error } = await q;
    if (error) throw error;
    document.getElementById("gameBlockOverlay").classList.remove("show");
    renderGamesPage();
  } catch (e) {
    console.error(e);
    msg.innerHTML = '<div class="form-msg err">ذخیره بازی ناموفق بود.</div>';
  }
}
async function saveMode() {
  ...
  if (!title || !editingModeBlockId) {
    msg.innerHTML = '<div class="form-msg err">اطلاعات مود کامل نیست.</div>';
    return;
  }
  const data = {
    block_id: editingModeBlockId,
    title,
    description: document.getElementById("moDesc").value.trim(),
    maps: document.getElementById("moMaps").value.trim(),
  };
  try {
    const q = editingModeId
      ? sb.from("game_modes").update(data).eq("id", editingModeId)
      : sb
          .from("game_modes")
          .insert([
            {
    ...
    const { error } = await q;
    if (error) throw error;
    document.getElementById("gameModeOverlay").classList.remove("show");
    renderGamesPage();
  } catch (e) {
    console.error(e);
    msg.innerHTML = '<div class="form-msg err">ذخیره مود ناموفق بود.</div>';
  }
}
async function deleteBlock(id) {
  if (!isStaff(currentUser) || !id) return;
  if (!confirm("این بازی و مودهای وابسته حذف بشن؟")) return;
  try {
    const { error: m } = await sb
      .from("game_modes")
      .delete()
      .eq("block_id", id);
    if (m) throw m;
...
async function renderHomeStats() {
  const stats = [
    ["statMembers", "accounts"],
    ["statGames", "game_blocks"],
    ["statModes", "game_modes"],
  ];
  await Promise.all(
    stats.map(async ([id, table]) => {
...
async function renderRegisterPage() {
  ...
  if (!form) return;
  if (form.dataset.kzCoreReady === "1") return;
  form.dataset.kzCoreReady = "1";
  form.addEventListener("submit", async (e) => {
    ...
    renderUserBox();
    msg.innerHTML =
      '<div class="form-msg ok">اکانت مهمانت ساخته شد. حالا درخواست عضویت بده. ✔</div>';
...
async function boot() {
  ...
  try {
    ...
    if (
      ...
      document.getElementById("statModes")
    )
      await renderHomeStats();
    if (document.getElementById("regForm")) await renderRegisterPage();
  } catch (e) {
    console.error("KillZone boot failed", e);
    document.querySelectorAll(".loading-note").forEach((x) => {
      x.textContent = "بارگذاری ناموفق بود؛ صفحه رو دوباره باز کن.";
    });
  }
}
