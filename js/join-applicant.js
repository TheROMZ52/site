(function(){
  ...
  const msg=(el,text,tone='err')=>{if(el)el.innerHTML=`<div class="form-msg ${tone}">${esc(text)}</div>`;};

  function loading(text='در حال بارگذاری...'){
    const root=page();
    if(root)root.innerHTML=`<div class="kz-join-shell"><div class="kz-community-loading">${esc(text)}</div></div>`;
  }
  ...
  function loginGate(){
    ...
    document.getElementById('kzJoinLogin')?.addEventListener('click',()=>document.getElementById('loginOpenBtn')?.click());
  }
  ...
  function formState(previous){
    const root=page();
    if(!root)return;
    const note=previous&&terminal.has(previous.status)?`<div class="form-msg err" style="margin-bottom:16px">درخواست قبلیت ${esc(kzStatusText(previous.status))} شده. می‌تونی یک درخواست جدید ثبت کنی.</div>`:'';
  ...
  async function submitRequest(e){
    ...
    try{
      const latest=typeof kzFindLatestMyRequest==='function'?await kzFindLatestMyRequest():null;
      ...
      const payload={
        ...
        account_id:u.id,
        status:'pending',
        message:[values.why_join,values.contribution].filter(Boolean).join('\n\n'),
      ...
      const {data:created,error}=await sb.from('team_join_requests').insert([payload]).select('*').maybeSingle();
      if(error)throw error;
      if(!created)throw new Error('درخواست ثبت نشد.');
      const accountUpdate=await sb.from('accounts').update({team_status:'pending'}).eq('id',u.id);
      if(accountUpdate.error)console.warn('KillZone applicant account status sync failed',accountUpdate.error);
      const initial=[payload.message].filter(Boolean).join('\n');
      if(initial){
        const {error:messageError}=await sb.from('team_join_messages').insert([{
          id:typeof kzNewId==='function'?kzNewId('msg'):'msg-'+Date.now(),
          request_id:created.id,
          account_id:u.id,
          sender_role:'applicant',
          message:initial
        }]);
        if(messageError)console.warn('KillZone applicant first message failed',messageError);
      }
      u.team_status='pending';
      if(typeof renderUserBox==='function')renderUserBox();
      if(typeof window.kzPresence?.refresh==='function')window.kzPresence.refresh();
  ...
  async function submitReply(e,request){
    e.preventDefault();
    const form=e.currentTarget,input=form?.querySelector('#kzReplyInput'),notice=form?.querySelector('#kzReplyMsg'),button=form?.querySelector('button[type="submit"]'),u=user();
    ...
    try{
      ...
      if(request.status==='waiting_applicant'){
        const {error:e2}=await sb.from('team_join_requests').update({status:'reviewing'}).eq('id',request.id).eq('account_id',u.id);
        if(e2)throw e2;
  ...
  async function renderJoin(fromRefresh){
    ...
    if(!u){loginGate();return;}
    if(u.team_status==='approved'||(typeof kzIsMember==='function'&&kzIsMember(u))){approvedState();return;}
    rendering=true;
    try{
      loading(fromRefresh?'در حال بروزرسانی تیکت...':'در حال بررسی وضعیت درخواست...');
      const latest=typeof kzFindLatestMyRequest==='function'?await kzFindLatestMyRequest():null;
      if(latest&&ACTIVE.has(latest.status)){await renderTicket(latest);return;}
      ...
      root.innerHTML='<div class="kz-join-shell"><div class="form-msg err">بارگذاری بخش عضویت ناموفق بود. دوباره تلاش کن.</div></div>';
    }finally{rendering=false;}
  }

  window.kzRenderJoinPage=renderJoin;
  const boot=async()=>{if(typeof initSession==='function'&&!window.currentUser){try{await initSession();}catch(_){}}await renderJoin(false);};
