...
(function(){
  ...
  async function getAnnouncements(limit){
    let q=sb.from('announcements').select('id,title,body,category,pinned,published_at,author_id').order('pinned',{ascending:false}).order('published_at',{ascending:false});
  ...
  function avatarHtml(m, large){
    const cls=large?'kz-profile-large-avatar':'kz-profile-avatar';
    return m.photo ? `<img class="${cls}" src="${esc(m.photo)}" alt="${esc(m.username)}" loading="lazy" decoding="async" onerror="this.outerHTML='<div class=\"${cls}\">${esc(initials(m.username))}</div>'">` : `<div class="${cls}">${esc(initials(m.username))}</div>`;
  }
  ...
  async function renderMembers(){
    ...
    accounts.forEach(m=>{
      ...
      const gameText=games.slice(0,2).join(' · ') || 'بازی ثبت نشده';
  ...
  async function ensureProfileModal(){
    let overlay=document.getElementById('kzProfileOverlay');
    if(overlay) return overlay;
    overlay=document.createElement('div'); overlay.className='overlay'; overlay.id='kzProfileOverlay';
  ...
  async function openProfile(m){
    const overlay=await ensureProfileModal();
    const content=document.getElementById('kzProfileContent');
    content.innerHTML=`<div class="kz-profile-hero">${avatarHtml(m,true)}<div><h3 id="kzProfileTitle">${esc(m.username)}</h3><p>${esc(m.bio || 'این عضو هنوز معرفی کوتاهی ثبت نکرده.')}</p>${safeUrl(m.social)?`<a class="kz-social" href="${esc(safeUrl(m.social))}" target="_blank" rel="noopener noreferrer">لینک پروفایل ↗</a>`:''}</div></div><section class="kz-profile-section"><div class="kz-profile-section-title"><h4>درباره عضو</h4><span>PROFILE</span></div><div class="kz-profile-meta"><span>${esc(String(m.game||'بازی ثبت نشده').replaceAll('،',' · '))}</span>${m.joined_at?`<span>از ${esc(dateFa(m.joined_at))}</span>`:''}</div></section><section class="kz-profile-section"><div class="kz-profile-section-title"><h4>دستاوردها</h4><span>ACHIEVEMENTS</span></div><div id="kzProfileAchievements" class="kz-achievements"><div class="kz-community-loading">در حال بارگذاری دستاوردها…</div></div></section>`;
    overlay.classList.add('show');
    const ach=await getAchievements(m.id); const box=document.getElementById('kzProfileAchievements');
    if(!box) return;
    box.innerHTML=ach.length?ach.map(a=>`<article class="kz-achievement"><div class="kz-achievement-icon">${esc(a.icon||'◆')}</div><div><h4>${esc(a.title)}</h4><p>${esc(a.description)}</p><small>${esc(dateFa(a.awarded_at))}</small></div></article>`).join(''):'<div class="kz-community-empty">هنوز دستاوردی ثبت نشده.</div>';
  }

  function renderNews(container, items){
    container.innerHTML=items.length?`<div class="kz-news-list">${items.map(n=>`<article class="kz-news-card ${n.pinned?'is-pinned':''}"><div class="kz-news-head"><div><h3 class="kz-news-title">${esc(n.title)}</h3><p class="kz-news-date">${esc(dateFa(n.published_at))}</p></div><span class="kz-news-badge">${esc(categoryLabel(n.category))}</span></div><p class="kz-news-body">${esc(n.body)}</p>${window.currentUser && typeof isStaff==='function' && isStaff(window.currentUser)?`<div class="kz-news-actions"><button class="btn ghost small kz-news-edit" data-id="${esc(n.id)}">ویرایش</button><button class="btn ghost small kz-news-delete" data-id="${esc(n.id)}">حذف</button></div>`:''}</article>`).join('')}</div>`:'<div class="kz-community-empty">هنوز خبر یا اطلاعیه‌ای منتشر نشده.</div>';
  }

  async function renderNewsPage(){
    const c=document.getElementById('newsContainer'); if(!c) return;
    c.innerHTML='<div class="kz-community-loading">در حال دریافت خبرها…</div>';
    const items=await getAnnouncements(); renderNews(c,items);
    bindNewsAdmin(c,items);
  }
  function bindNewsAdmin(container,items){
    if(!(window.currentUser && typeof isStaff==='function' && isStaff(window.currentUser))) return;
    document.getElementById('newsAddBtn')?.addEventListener('click',()=>openNewsEditor());
    container.querySelectorAll('.kz-news-edit').forEach(b=>b.addEventListener('click',()=>openNewsEditor(items.find(x=>x.id===b.dataset.id))));
    container.querySelectorAll('.kz-news-delete').forEach(b=>b.addEventListener('click',async()=>{
      if(!confirm('این خبر حذف بشه؟')) return;
      const {error}=await sb.from('announcements').delete().eq('id',b.dataset.id); if(error){alert('حذف خبر انجام نشد.');return;} renderNewsPage();
    }));
  }
  async function openNewsEditor(item){
    let o=document.getElementById('kzNewsEditor');
    if(!o){ o=document.createElement('div');o.className='overlay';o.id='kzNewsEditor';o.innerHTML='<div class="modal kz-modal-panel"><button class="close" id="kzNewsClose">✕</button><h3 id="kzNewsEditorTitle">افزودن خبر</h3><div class="form-grid"><div class="field"><label>عنوان</label><input id="kzNewsTitle" maxlength="120"></div><div class="field"><label>نوع</label><select id="kzNewsCategory"><option value="news">خبر</option><option value="announcement">اطلاعیه</option><option value="event">رویداد</option></select></div><div class="field"><label>متن</label><textarea id="kzNewsBody" rows="6" maxlength="4000"></textarea></div><label class="field"><span><input type="checkbox" id="kzNewsPinned"> سنجاق شود</span></label><button class="btn primary" id="kzNewsSave">ذخیره</button><div id="kzNewsMsg"></div></div></div>';document.body.appendChild(o);o.addEventListener('click',e=>{if(e.target===o)o.classList.remove('show');});document.getElementById('kzNewsClose').addEventListener('click',()=>o.classList.remove('show')); }
    o.dataset.id=item?.id||'';document.getElementById('kzNewsEditorTitle').textContent=item?'ویرایش خبر':'افزودن خبر';document.getElementById('kzNewsTitle').value=item?.title||'';document.getElementById('kzNewsCategory').value=item?.category||'news';document.getElementById('kzNewsBody').value=item?.body||'';document.getElementById('kzNewsPinned').checked=!!item?.pinned;document.getElementById('kzNewsMsg').innerHTML='';o.classList.add('show');
    const btn=document.getElementById('kzNewsSave'); btn.onclick=async()=>{ const title=document.getElementById('kzNewsTitle').value.trim(),body=document.getElementById('kzNewsBody').value.trim(); if(!title||!body){document.getElementById('kzNewsMsg').innerHTML='<div class="form-msg err">عنوان و متن رو کامل کن.</div>';return;} const data={title,body,category:document.getElementById('kzNewsCategory').value,pinned:document.getElementById('kzNewsPinned').checked}; let q=o.dataset.id?sb.from('announcements').update(data).eq('id',o.dataset.id):sb.from('announcements').insert([{id:'news-'+Date.now()+'-'+Math.random().toString(36).slice(2,7),...data,author_id:window.currentUser?.id||null}]);const {error}=await q;if(error){document.getElementById('kzNewsMsg').innerHTML='<div class="form-msg err">ذخیره خبر ناموفق بود.</div>';return;}o.classList.remove('show');renderNewsPage(); };
  }

  async function boot(){
    if(typeof sb==='undefined'){setTimeout(boot,120);return;}
    await new Promise(r=>setTimeout(r,0));
    if(document.getElementById('membersContainer')) renderMembers();
