...
(function () {
  ...
  const limits = { username:24, password:128, game:120, bio:500, social:1000, title:120, description:4000, body:4000, status_text:100 };
  const classify = (el) => {
    const id=String(el.id||"").toLowerCase(), name=String(el.name||"").toLowerCase(), type=String(el.type||"").toLowerCase();
    if(type==="password"||id.includes("pass")||name.includes("pass")) return limits.password;
    if(id.includes("user")||id.includes("name")||name==="username") return limits.username;
    ...
    if(id.includes("statustext")||id.includes("status_text")) return limits.status_text;
    if(id.includes("title")) return limits.title;
    if(id.includes("description")||id.includes("desc")||id.includes("body")) return limits.description;
    if(el.tagName==="TEXTAREA") return limits.body;
    return null;
  };
  const guard=(el)=>{ if(!(el instanceof HTMLInputElement||el instanceof HTMLTextAreaElement)) return; const max=classify(el); if(max&&el.value.length>max) el.value=el.value.slice(0,max); };
  document.addEventListener("input",e=>guard(e.target),true);
  document.addEventListener("change",e=>guard(e.target),true);
  document.addEventListener("submit",e=>e.target?.querySelectorAll?.("input,textarea").forEach(guard),true);
  function wrap(name,event,onResolve){
    const original=window[name];
    if(typeof original!=="function"||original.__kzLifecycleWrapped) return;
    const wrapped=function(){
      const result=original.apply(this,arguments);
      if(result&&typeof result.then==="function") return result.then(v=>{emit(event);onResolve?.(v);return v;},e=>{throw e;});
      emit(event); onResolve?.(result); return result;
    };
    wrapped.__kzLifecycleWrapped=true; window[name]=wrapped;
  }
  document.addEventListener("DOMContentLoaded",()=>{wrap("saveSession","kz:session-changed");wrap("clearSession","kz:session-changed");wrap("initSession","kz:session-ready");},{once:true});
})();

(function () {
  "use strict";
  const install=()=>{
    const nav=document.querySelector("nav.main"), header=document.querySelector("header");
    if(!nav||!header) return;
    const links=[["/chat","چت تیم"],["/news","اخبار"]];
    for(const [href,text] of links){
      if(!nav.querySelector(`a[href="${href}"]`)){
        const link=document.createElement("a"); link.href=href; link.textContent=text;
        const join=nav.querySelector('a[href="/join"]'); nav.insertBefore(link,join||null);
      }
    }
    ...
    if(menu){menu.style.position="relative";menu.style.zIndex="10003";}
