...
(function () {
  ...
  const isStaff = () => { const user = current(); return !!(user && STAFF.has(user.rank)); };
  const text = v => String(v ?? '').trim();
  const norm = v => text(v).toLocaleLowerCase('fa-IR');
  const esc = v => typeof window.escapeHtml === 'function'
    ? window.escapeHtml(v)
    : text(v).replace(/[&<>\"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
  const gameList = a => text(a?.game).split(/[،,|]/).map(text).filter(Boolean);
  const rankMeta = key => RANKS.find(r => r.key === key) || { key, label: key || 'سایر' };
  const chevrons = key => {
    const all = ['guest', 'new_member', 'member', 'admin', 'developer', 'co_owner', 'owner'];
  ...
  function stats() {
    const uniqueGames = new Set(accounts.flatMap(gameList).map(norm).filter(Boolean));
  ...
  function avatarHtml(account, large = false) {
    ...
    const initials = esc((account.username || '?').trim().slice(0, 2).toUpperCase());
  ...
  function hydrateAvatars(scope) {
    scope?.querySelectorAll('.kz-avatar-media[data-avatar-url]').forEach(el => {
  ...
  function memberCard(account) {
    ...
    card.querySelector('.kz-member-edit')?.addEventListener('click', event => {
      event.stopPropagation();
      if (typeof window.wireMemberModal === 'function') window.wireMemberModal();
      if (typeof window.openMemberModal === 'function') window.openMemberModal(account.id, accounts);
    });
    card.querySelector('.kz-member-delete')?.addEventListener('click', async event => {
      event.stopPropagation();
  ...
  function renderGroups() {
    ...
    const selectedGame = norm(document.getElementById('memberGameFilter')?.value || 'all');
    ...
    const visibleAccounts = accounts.filter(account => {
      const hay = norm([account.username, account.bio, account.game, account.rank].join(' '));
      const qOk = !query || hay.includes(query);
      const gOk = selectedGame === 'all' || gameList(account).some(g => norm(g) === selectedGame);
      const sOk = status === 'all' || (status === 'online' ? online(account.id) : !online(account.id));
      return qOk && gOk && sOk;
    });
    ...
    let groupCount = 0;
    RANKS.forEach(rank => {
      const members = visibleAccounts.filter(a => a.rank === rank.key).sort(compare);
    ...
    const other = visibleAccounts.filter(a => !known.has(a.rank)).sort(compare);
    if (other.length) {
      groupCount++;
      const section = document.createElement('section');
      section.className = 'kz-rank-section';
  ...
  function fallbackProfile(account) {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = '<div class="modal kz-modal-panel"><button class="close" type="button" aria-label="بستن">✕</button><div class="kz-fallback-profile"></div></div>';
  ...
  function wire() {
    ...
    clear?.addEventListener('click', () => { if (!search) return; search.value = ''; renderGroups(); search.focus(); });
  ...
  async function guestManager() {
    ...
    if (!overlay) {
      ...
      overlay.className = 'overlay kz-community-overlay';
      overlay.innerHTML = '<div class="modal kz-modal-panel" role="dialog" aria-modal="true"><button class="close" id="kzGuestClose" type="button">✕</button><div class="kz-profile-section-title"><h4>اکانت‌های مهمان</h4><span>GUEST ACCOUNTS</span></div><div id="kzGuestList"></div></div>';
      document.body.appendChild(overlay);
      overlay.addEventListener('click', e => { if (e.target === overlay) overlay.classList.remove('show'); });
    ...
    box.querySelectorAll('.kz-runtime-guest-card').forEach(card => {
      ...
      card.querySelector('[data-action="delete"]')?.addEventListener('click', async event => {
        ...
        if (b.dataset.confirm !== '1') {
          b.dataset.confirm = '1'; b.textContent = 'تأیید حذف';
