...
(function(){
  ...
  function feedback(message,tone='err'){
    ...
    if(!box){
      ...
      box.style.cssText='position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:1000;max-width:min(92vw,520px);padding:11px 16px;border:1px solid var(--line,#3b362a);background:rgba(16,16,14,.96);color:var(--paper,#f4eddc);font:600 13px/1.7 Vazirmatn,Arial,sans-serif;box-shadow:0 16px 40px rgba(0,0,0,.35);opacity:0;pointer-events:none;transition:opacity .18s ease,border-color .18s ease}';
      document.body.appendChild(box);
    }
    box.textContent=message;
    box.style.borderColor=tone==='ok'?'var(--grass,#728b4b)': 'var(--ember,#df6330)';
    box.style.opacity='1';
    clearTimeout(box._timer);
    box._timer=setTimeout(()=>{box.style.opacity='0'},3200);
  }

  async function deleteAccountCascade(id){
    if(!isStaff()||!window.sb||!id)return false;
    if(window.currentUser?.id===id){feedback('نمی‌تونی اکانت خودت رو از اینجا حذف کنی.','err');return false;}
    const related=[['team_join_messages','account_id'],['team_join_requests','account_id'],['member_presence','account_id'],['achievements','account_id']];
  ...
  async function approveGuest(id,overlay){
    ...
    const {error}=await sb.from('accounts').update({rank:'member',team_status:'approved'}).eq('id',id).eq('rank','guest');
    if(error){console.error('KillZone guest approve',error);feedback('تأیید مهمان انجام نشد.');return;}
    ...
    box.addEventListener('click',async e=>{
      ...
      if(button.dataset.action==='delete'){
        if(button.dataset.confirm!=='1'){
          ...
          button._resetTimer=setTimeout(()=>{button.dataset.confirm='0';button.textContent='حذف';button.classList.remove('danger')},3500);
          return;
        }
        button.disabled=true;
        try{
  ...
  function bindMemberActions(){
    ...
    root.addEventListener('click',async e=>{
      ...
      if(!id)return;
      if(window.currentUser?.id===id){feedback('نمی‌تونی اکانت خودت رو از اینجا حذف کنی.');return;}
      if(deleteButton.dataset.confirm!=='1'){
        ...
        deleteButton._resetTimer=setTimeout(()=>{deleteButton.dataset.confirm='0';deleteButton.textContent='حذف';deleteButton.classList.remove('danger')},3500);
        return;
      }
  ...
  function bindAddMember(){
    ...
    button.addEventListener('click',e=>{
      ...
      if(typeof window.openMemberModal==='function')window.openMemberModal(null,[]);
    },true);
  }
  ...
  function bindEscapeForMemberOverlays(){
    ...
    document.addEventListener('keydown',e=>{
      ...
      ['kzGuestManager','kzProfileOverlayFallback','memberOverlay'].forEach(id=>document.getElementById(id)?.classList.remove('show'));
    },true);
  }
  ...
  function boot(){
    ...
    window.addEventListener('pagehide',()=>{observer.disconnect();clearInterval(timer)});
  }
