...
(function () {
  ...
  function patchRegistration() { if (typeof window.registerUser !== "function" || window.registerUser.__kzRuntimeFix) return; const original = window.registerUser; const wrapped = async function (username, password, game, photo) { const name = String(username || "").trim(); const pass = String(password || ""); if (name.length < 3 || name.length > 32 || !USERNAME_RE.test(name)) return { ok: false, msg: "نام‌کاربری باید ۳ تا ۳۲ کاراکتر و فقط شامل حروف انگلیسی، عدد، _، - یا . باشه." }; if (pass.length < 6) return { ok: false, msg: "رمز عبور باید حداقل ۶ کاراکتر باشه." }; return original(name, pass, game, photo); }; wrapped.__kzRuntimeFix = true; window.registerUser = wrapped; }
  ...
  function boot() {
    injectButtonStyles(); injectShellFixes(); injectTwemoji(); patchUnsafeSeed(); patchUploadPhoto(); patchRegistration(); patchLoginUX(); patchMembersFilter(); fixHomeStats(); fixBrokenScriptDependencies(); fixChatSafetyConflict(); fixMembershipNavigation(); setTimeout(syncApplicantStatusFromTicket, 250);
  }
