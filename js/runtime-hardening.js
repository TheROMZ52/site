...
(function () {
  ...
  function injectTwemoji() {
    ...
    style.textContent =
      ".kz-twemoji, img.emoji, .kz-lion-sun{display:inline-block;width:1em;height:1em;margin:0 .05em 0 .1em;vertical-align:-0.1em;line-height:1;object-fit:contain;}";
    ...
    script.integrity =
      "sha384-Y5xukbGJwykbHHkTbLJykYLcBPFxrwipTbEh0puxhkz9CZ90raTPGe2Ks4vCxsYU";
    ...
    while ((current = walk.nextNode())) {
      if (
        ...
        current.parentElement.closest(
          'script,style,textarea,input,[contenteditable="false"]',
        )
      )
        continue;
      if (current.nodeValue && current.nodeValue.includes("🇮🇷"))
  ...
  function startTwemoji() {
    ...
    const parseNode = (node) => {
      ...
      try {
        ...
        if (node.nodeType === Node.ELEMENT_NODE)
          window.twemoji.parse(node, options);
        else if (node.nodeType === Node.TEXT_NODE && node.parentElement)
          window.twemoji.parse(node.parentElement, options);
      } catch (e) {
        console.warn("KillZone Twemoji parse skipped", e);
      }
    };
    ...
    try {
      ...
      console.warn("KillZone Twemoji initial parse skipped", e);
    }
  ...
  function patchUnsafeSeed() {
    if (
      typeof window.ensureSeedAccounts !== "function" ||
      window.ensureSeedAccounts.__kzSafeSeed
    )
      return;
  ...
  function patchUploadPhoto() {
    if (
      typeof window.uploadPhoto !== "function" ||
      window.uploadPhoto.__kzImageFix
    )
      return;
    ...
    const wrapped = async function (file, onStatus) {
      if (file && !ALLOWED_IMAGE_TYPES.has(file.type)) {
        if (typeof onStatus === "function")
          onStatus("err", "فقط JPG، PNG یا WEBP مجازه.");
        return null;
      }
      if (file && file.size > 5 * 1024 * 1024) {
        if (typeof onStatus === "function")
          onStatus("err", "حجم عکس باید کمتر از ۵ مگابایت باشه.");
        return null;
      }
  ...
  function patchRegistration() {
    if (
      ...
      window.registerUser.__kzRuntimeFix
    )
      return;
    ...
    const wrapped = async function (username, password, game, photo) {
      ...
      if (name.length < 3 || name.length > 32 || !USERNAME_RE.test(name))
        return {
          ok: false,
          msg: "نام‌کاربری باید ۳ تا ۳۲ کاراکتر و فقط شامل حروف انگلیسی، عدد، _، - یا . باشه.",
        };
      if (pass.length < 6)
  ...
  function patchMembersFilter() {
    ...
    if (
      ...
      window.fetchAccounts.__kzApprovedOnly
    )
      return;
  ...
  function syncRenderedAccountStatus(status) {
    ...
    const meta = document.querySelectorAll(
      "#kzAccountApp .profile-meta strong",
    );
