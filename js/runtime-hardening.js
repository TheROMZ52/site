...
(function () {
  ...
  function injectButtonStyles() {
    ...
    link.rel = "stylesheet";
    link.href = "/css/buttons.css?v=20260910";
  ...
  function injectShellFixes() {
    ...
    style.id = "kz-shell-fixes";
    style.textContent = `#userBox #logoutBtn{display:none!important}.btn,.link-btn,.admin-btn,.menu-toggle{visibility:visible!important;opacity:1!important}`;
  ...
  function startTwemoji() {
    ...
    const observer = new MutationObserver((mutations) => { for (const mutation of mutations) for (const node of mutation.addedNodes) if (node.nodeType === Node.TEXT_NODE || node.nodeType === Node.ELEMENT_NODE) parseNode(node); });
  ...
  function patchLoginUX() {
    ...
    [document.getElementById("loginUser"), document.getElementById("loginPass")].filter(Boolean).forEach((input) => input.addEventListener("keydown", (e) => { if (e.key === "Enter" && submit) { e.preventDefault(); submit.click(); } }));
  ...
  async function syncApplicantStatusFromTicket() {
    try {
      ...
      const { data, error } = await sb.from("team_join_requests").select("status").eq("account_id", u.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (error || !data?.status || data.status === u.team_status) return;
      u.team_status = data.status;
      try { localStorage.setItem("kz_session", JSON.stringify(u)); } catch {}
