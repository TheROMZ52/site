...
(function(){
  ...
  const GAMES = [
    {
      ...
      single: {
        ...
        href: '/games/minecraft.html',
        action: 'اجرای بازی'
      },
      multi: {
        title: 'Multiplayer SMP',
    ...
    },
    {
      ...
      single: {
        ...
        href: '/games/counter-strike.html',
        action: 'اجرای بازی'
      },
      multi: {
        title: 'Multiplayer Respawn',
        status: 'در حال آماده‌سازی',
  ...
  function modeCard(game, mode, kind){
    ...
    const action = mode.href
      ? `<a class="btn ${kind === 'multi' ? 'ghost' : 'primary'} small kz-hub-action" href="${escapeHtml(mode.href)}">${escapeHtml(mode.action)} <span aria-hidden="true">←</span></a>`
      : `<button class="btn ghost small kz-hub-action" type="button" disabled>${escapeHtml(mode.action)}</button>`;
    ...
    return `
      <article class="kz-hub-mode ${kind}">
        <div class="kz-hub-mode-top">
          ...
          <span class="kz-hub-status ${mode.disabled ? 'soon' : 'live'}">
            <i></i>${escapeHtml(mode.status)}
          </span>
        </div>
        <p>${escapeHtml(mode.description)}</p>
        ${bullets ? `<div class="kz-hub-chips">${bullets}</div>` : ''}
        <div class="kz-hub-mode-footer">${action}</div>
      </article>
    `;
  }
  ...
  function render(){
    ...
    if(!container) return;
    container.innerHTML = `
      <div class="kz-hub-grid">
        ${GAMES.map(gameCard).join('')}
      </div>
      <div class="kz-hub-note">
        <span class="kz-hub-note-mark">KZ //</span>
        <p>بخش Multiplayer فعلاً در حال آماده‌سازی زیرساخت آنلاین است؛ وقتی سرورهای واقعی وصل شوند، همین کارت‌ها محل ورود به مچ و SMP خواهند بود.</p>
      </div>
    `;
  }
  ...
  function boot(){
    render();
    document.documentElement.classList.add('kz-hub-ready');
  }
