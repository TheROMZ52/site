...
function kzApplyJoinNav(){
  document.querySelectorAll('nav.main').forEach(nav=>{
    ...
    if(!reg) return;
    const a=document.createElement('a'); a.href='join.html'; a.textContent='عضویت در تیم';
...
function kzMessageHtml(m, request){
  ...
  const sender=isAdmin ? 'مدیریت KillZone' : (request?.first_name || request?.accounts?.username || 'متقاضی');
...
function kzRequestFormHtml(){
  return `
  <form id="kzJoinForm" class="kz-join-form">
    <div class="kz-form-section"><div class="kz-form-kicker">01 // اطلاعات پایه</div><h3>اول با خودت آشنا بشیم</h3>
    ...
    <div class="kz-form-section"><div class="kz-form-kicker">02 // پروفایل گیمینگ</div><h3>سبک بازی و فعالیتت</h3>
      <div class="kz-form-grid">
    ...
    <div class="kz-form-section"><div class="kz-form-kicker">03 // خودت و KillZone</div><h3>بیشتر از خودت بگو</h3>
      <div class="kz-form-stack">
    ...
    <div class="kz-form-section"><div class="kz-form-kicker">04 // تأیید</div><h3>آخرش فقط یک تأیید</h3>
      <label class="kz-check"><input id="joinInfoConfirmed" type="checkbox" required><span>تأیید می‌کنم اطلاعاتی که در این فرم وارد کردم واقعی و متعلق به خودم است.</span></label>
    </div>
...
function kzTicketHeaderHtml(r){
  const number=r?.id ? r.id.replace(/^req-/,'').slice(-8).toUpperCase() : '--------';
...
function kzFormSummaryHtml(r){
  const rows=[
    ['نام',r.first_name],['نام خانوادگی',r.last_name],['نام کاربری',r.accounts?.username || currentUser?.username],['آیدی روبیکا',r.rubika_id],['سن',r.age],['شهر',r.city],
    ['بازی‌ها',r.other_games],['سطح',r.skill_level],['سابقه بازی',r.gaming_years!=null?`${r.gaming_years} سال`:'' ],['فعالیت هفتگی',r.weekly_activity],['Voice Chat',r.voice_chat],
    ['چرا KillZone؟',r.why_join],['چه چیزی اضافه می‌کنی؟',r.contribution],['اگر اختلاف پیش بیاد؟',r.conflict_response],['نحوه آشنایی',r.how_found_us]
  ];
...
async function kzSubmitJoinForm(form){
  ...
  const values=kzRequestFormValues(form); const errors=kzValidateJoinForm(values);
  if(errors.length){ msg.innerHTML=`<div class="form-msg err">${escapeHtml(errors[0])}</div>`; return; }
  const latest=await kzFindLatestMyRequest();
  if(latest && KZ_ACTIVE_TICKET_STATUSES.includes(latest.status)){
    location.href='join.html'; return;
  }
  ...
  const {error:accError}=await sb.from('accounts').update({team_status:'reviewing'}).eq('id',currentUser.id);
  if(accError) console.error(accError);
  currentUser.team_status='reviewing';
  msg.innerHTML='<div class="form-msg ok">درخواستت ثبت شد. الان وارد تیکتت می‌شیم. 🎫</div>';
  renderUserBox(); kzRefreshMembershipUI();
  setTimeout(()=>kzRenderJoinPage(data||payload),250);
}

async function kzRenderMyView(root){
  const request=await kzFindLatestMyRequest();
  if(request && KZ_ACTIVE_TICKET_STATUSES.includes(request.status)){
    const {data:messages}=await kzLoadMessages(request.id);
    root.innerHTML=`<div class="kz-view-head"><div><div class="section-title top"><h2>تیکت عضویت من</h2><p>اینجا گفت‌وگوی تو با مدیریت KillZone ادامه پیدا می‌کنه.</p></div></div></div><section class="kz-ticket panelish">${kzTicketHeaderHtml(request)}${kzFormSummaryHtml(request)}${kzTicketThreadHtml(messages,request)}${kzTicketReplyHtml(true)}</section>`;
    kzWireReply(root,request);
    return;
  }
  if(request && request.status==='approved'){
    const {data:messages}=await kzLoadMessages(request.id);
    root.innerHTML=`<div class="section-title top"><h2>عضویت تأیید شد 🎉</h2><p>به KillZone خوش اومدی. تیکتت برای سابقه درخواست نگه داشته شده.</p></div><section class="kz-ticket panelish">${kzTicketHeaderHtml(request)}${kzFormSummaryHtml(request)}${kzTicketThreadHtml(messages,request)}</section>`;
    return;
  }
  if(request && request.status==='rejected'){
    const {data:messages}=await kzLoadMessages(request.id);
    root.innerHTML=`<div class="section-title top"><h2>درخواست قبلی رد شده</h2><p>گفت‌وگوی قبلی را ببین یا یک درخواست تازه ثبت کن.</p></div><section class="kz-ticket panelish kz-rejected-ticket">${kzTicketHeaderHtml(request)}${kzFormSummaryHtml(request)}${kzTicketThreadHtml(messages,request)}</section><div class="kz-new-request-wrap"><button class="btn primary" id="kzNewRequestBtn">ثبت درخواست جدید</button></div>`;
    root.querySelector('#kzNewRequestBtn').addEventListener('click',()=>{ root.innerHTML=kzRequestFormHtml(); kzWireJoinForm(root); });
    return;
  }
  root.innerHTML=`<div class="section-title top"><h2>درخواست عضویت در KillZone</h2><p>قبل از ورود به تیم، چند سؤال کوتاه داریم تا باهات آشنا بشیم.</p></div>${kzRequestFormHtml()}`;
  kzWireJoinForm(root);
}

async function kzRenderAdminList(root){
  const list=document.createElement('div'); list.className='kz-admin-layout';
  list.innerHTML='<aside class="kz-ticket-list"><div class="kz-ticket-list-head"><div><div class="kz-eyebrow">STAFF // APPLICATIONS</div><h3>درخواست‌ها</h3></div><span class="kz-list-count">…</span></div><div class="kz-list-items">در حال بارگذاری...</div></aside><section class="kz-admin-detail" id="kzAdminDetail"><div class="kz-admin-placeholder">یک درخواست را از لیست انتخاب کن.</div></section>';
  root.innerHTML='<div class="section-title top"><h2>مرکز درخواست‌های عضویت</h2><p>فقط Developer، Co-Owner و Owner به این بخش دسترسی دارند.</p></div>';
  root.appendChild(list);
  const {data,error}=await sb.from('team_join_requests').select('*,accounts(username,photo,rank)').order('created_at',{ascending:false});
  if(error){ list.querySelector('.kz-list-items').innerHTML='<div class="form-msg err">خطا در بارگذاری درخواست‌ها.</div>'; return; }
  const items=list.querySelector('.kz-list-items'); list.querySelector('.kz-list-count').textContent=data?.length||0;
  if(!data?.length){ items.innerHTML='<div class="kz-list-empty">درخواستی ثبت نشده.</div>'; return; }
  items.innerHTML='';
  data.forEach(r=>{
    const btn=document.createElement('button'); btn.type='button'; btn.className='kz-ticket-list-item'; btn.dataset.id=r.id;
    btn.innerHTML=`<span class="kz-list-status ${kzStatusClass(r.status)}"></span><span class="kz-list-copy"><strong>${escapeHtml(r.first_name||r.accounts?.username||'بدون نام')}</strong><small>@${escapeHtml(r.accounts?.username||'—')} · ${escapeHtml(kzStatusText(r.status))}</small></span><time>${escapeHtml(kzFormatDate(r.created_at))}</time>`;
    btn.addEventListener('click',()=>kzOpenAdminTicket(r,list)); items.appendChild(btn);
  });
  kzOpenAdminTicket(data[0],list);
}

async function kzOpenAdminTicket(request,listRoot){
  listRoot.querySelectorAll('.kz-ticket-list-item').forEach(x=>x.classList.toggle('active',x.dataset.id===request.id));
  const detail=listRoot.querySelector('#kzAdminDetail');
  detail.innerHTML='<div class="kz-admin-placeholder">در حال بارگذاری تیکت...</div>';
  const {data:messages}=await kzLoadMessages(request.id);
  ...
  detail.querySelector('#kzApproveBtn')?.addEventListener('click',()=>kzReviewTicket(request,'approved',listRoot));
  detail.querySelector('#kzRejectBtn')?.addEventListener('click',()=>kzReviewTicket(request,'rejected',listRoot));
  kzWireReply(detail,request);
}
...
async function kzReviewTicket(request,decision,listRoot){
  ...
  const {error:reqError}=await sb.from('team_join_requests').update({status:next,reviewed_at:new Date().toISOString(),reviewed_by:currentUser.id}).eq('id',request.id);
  if(reqError){ console.error(reqError); return; }
  const {error:accError}=await sb.from('accounts').update({team_status:next}).eq('id',request.account_id);
  if(accError) console.error(accError);
  ...
  kzOpenAdminTicket(request,listRoot);
}

function kzWireReply(scope,request){
  const form=scope.querySelector('#kzReplyForm'); if(!form) return;
  form.addEventListener('submit',async e=>{
    e.preventDefault();
    const input=form.querySelector('#kzReplyInput'); const msg=form.querySelector('#kzReplyMsg');
    const text=input.value.trim(); if(!text){ msg.innerHTML='<div class="form-msg err">پیام رو بنویس.</div>'; return; }
    form.querySelector('button').disabled=true;
    const res=await kzSendMessage(request,text);
    form.querySelector('button').disabled=false;
    if(!res.ok){ msg.innerHTML=`<div class="form-msg err">${escapeHtml(res.msg)}</div>`; return; }
    input.value=''; msg.innerHTML='<div class="form-msg ok">پیام ارسال شد.</div>';
    const parent=scope.closest('.kz-ticket'); if(parent){
      const loaded=await kzLoadMessages(request.id);
      const thread=parent.querySelector('.kz-thread');
      if(thread) thread.outerHTML=kzTicketThreadHtml(loaded.data,request);
      const {data:latest}=await sb.from('team_join_requests').select('status,reviewed_at').eq('id',request.id).maybeSingle();
      if(latest) request.status=latest.status;
      const head=parent.querySelector('.kz-ticket-head'); if(head) head.outerHTML=kzTicketHeaderHtml(request);
    }
    if(document.querySelector('.kz-admin-layout')) kzRenderAdminList(document.getElementById('kzJoinApp'));
  });
}

function kzWireJoinForm(scope){
  const form=scope.querySelector('#kzJoinForm'); if(!form) return;
  form.addEventListener('submit',e=>kzSubmitJoinForm(form));
}

async function kzRenderJoinPage(existing){
  const root=document.getElementById('kzJoinApp'); if(!root) return;
  if(!currentUser){
    root.innerHTML='<div class="section-title top"><h2>برای درخواست باید وارد شوی</h2><p>اول وارد اکانتت شو؛ بعد فرم عضویت در تیم را پر می‌کنی.</p></div><div class="panel kz-login-gate"><div class="kz-gate-icon">🔒</div><h3>حساب کاربری لازمه</h3><p>اکانت داری؟ از دکمه ورود بالای صفحه استفاده کن. هنوز اکانت نداری؟ اول ثبت‌نام کن.</p><div class="kz-gate-actions"><button class="btn primary" id="kzJoinLoginBtn">ورود</button><a class="btn ghost" href="register.html">ساخت اکانت</a></div></div>';
    root.querySelector('#kzJoinLoginBtn').addEventListener('click',()=>document.getElementById('loginOpenBtn')?.click());
    return;
  }
  if(kzIsReviewer(currentUser)){
    await kzRenderAdminList(root);
    return;
  }
  if(kzIsMember(currentUser)){
    root.innerHTML='<div class="section-title top"><h2>تو همین حالا عضو KillZone هستی 👑</h2><p>برای مدیریت اطلاعاتت از صفحه اعضا استفاده کن.</p></div><a class="btn primary" href="members.html">مشاهده اعضا</a>';
    return;
  }
...
document.addEventListener('submit',async e=>{
  ...
  msg.innerHTML='<div class="form-msg ok">اکانت ساخته شد! حالا از «عضویت در تیم» فرم درخواستت رو پر کن. 🎫</div>';
  renderUserBox(); kzRefreshMembershipUI();
},true);
...
document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  kzApplyJoinNav();
  kzPatchUserBox();
  if(document.getElementById('membersContainer') && typeof renderMembersPage==='function') renderMembersPage();
  ...
  if(document.getElementById('kzJoinApp')) kzRenderJoinPage();
},0));
